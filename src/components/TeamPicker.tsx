...
import { Goal, Star, X, Check, Pencil } from 'lucide-react';
...
export default function TeamPicker({ onSelect, onClose }: TeamPickerProps) {
  const [mode, setMode] = useState<'time' | 'meus'>('time');
  const [selectedCompetition, setSelectedCompetition] = useState('brasileirao-serie-a');
  ...
  const selectedCompetitionEntry = selectedCompetition
  ...
  if (mode === 'time') {
    if (selectedCompetitionEntry) {
      const names = new Set(selectedCompetitionEntry.teamNames);
  ...
  const comingSoon = mode === 'time' && !!selectedCompetitionEntry && selectedCompetitionEntry.teamNames.length === 0;

  useEffect(() => {
    if (mode !== 'meus') {
      prefetchLogos(teams.map(t => t.name));
    }
  }, [teams.length, mode]);
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 0, borderRadius: 'var(--radius)', overflow: 'hidden', border: '1px solid var(--border)' }}>
            <TogBtn active={mode === 'time'} onClick={() => { setMode('time'); setSelectedCompetition('brasileirao-serie-a'); setEditing(null); }}><Goal size={13} /> Time</TogBtn>
        ...
        {mode === 'meus' && editing ? (
          ...
          <div style={{ padding: '16px 20px', overflowY: 'auto', flex: 1 }}>
            <button onClick={() => setEditing('new')} style={{
              width: '100%', padding: '14px', borderRadius: 'var(--radius)', border: '2px dashed var(--green)',
          ...
          <div style={{
            ...
            {teams.length === 0 ? (
              <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: 30, color: 'var(--text3)', fontSize: 13 }}>
                {comingSoon ? 'Em breve!' : 'Nenhum time encontrado.'}
              </div>
            ) : teams.map((t, i) => (
              <div
                key={`${t.s}-${i}`}
                ...
                style={{
                  ...
                  cursor: 'pointer', padding: '14px 8px', borderRadius: 'var(--radius)',
...
function ActionBtn({ onClick, color, title, children }: { onClick: () => void; color: string; title: string; children: React.ReactNode }) {
  return (
    <button onClick={onClick} title={title} style={{
      width: 30, height: 30, borderRadius: 'var(--radius)', border: `1px solid ${color}33`,
